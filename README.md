...
Email + password auth backed by JWTs. Passwords are hashed with bcrypt; older
SHA‑256 hashes are still accepted and transparently upgraded on the next login.

### Student dashboard

![Dashboard](docs/screenshots/03-dashboard.png)

Landing view for a student: cognitive focus score, learning-activity summary,
and every study room they have created or joined. Create a room or join one with
a 6‑character code.

### Study room

![Study room — whiteboard](docs/screenshots/04-study-room.png)

The collaborative workspace. Left: real‑time room chat. Centre: a tabbed
workspace (shared whiteboard, quizzes, flashcards, code editor). Right: the
Pomodoro-style schedule, a live cognitive-load gauge fed by webcam/slider input,
and the AI co‑pilot.

| Quizzes | Take a quiz |
|---|---|
| ![Quizzes](docs/screenshots/05-quizzes.png) | ![Quiz attempt](docs/screenshots/06-quiz-attempt.png) |

| Flashcards | Code editor |
|---|---|
| ![Flashcards](docs/screenshots/07-flashcards.png) | ![Code editor](docs/screenshots/08-code-editor.png) |

Quizzes can be authored by hand or generated by an LLM; attempts are auto‑graded
and logged. Flashcard decks are per‑room with flip-to-reveal cards. The code
editor gives a shared scratch space with syntax highlighting.

### Analytics

| Personal analytics | Room analytics |
|---|---|
| ![Learning analytics](docs/screenshots/09-analytics.png) | ![Room analytics](docs/screenshots/10-room-analytics.png) |

Per‑student burnout risk and projected grade come from trained scikit‑learn
models (with a heuristic fallback and honest source reporting). Room analytics
aggregate interactions by type and by member, average group cognitive load, and
...
|---|---|
| ![Mentor dashboard](docs/screenshots/11-mentor-dashboard.png) | ![Admin dashboard](docs/screenshots/12-admin-dashboard.png) |

Role-gated views. Mentors monitor the rooms they own — member counts,
interactions, and high‑risk-student totals. Admins get platform-wide totals and
a user-role breakdown.

### Profile

![Profile](docs/screenshots/13-profile.png)

Account details, role/access level, and per‑viewer preferences.
